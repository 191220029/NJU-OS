...
void syscallReadStdIn(struct StackFrame *sf) {
	// TODO: complete `stdin`
	if(dev[STD_IN].value < 0){ //there is one process blocked
	...
	//no blocked process
		...
		dev[STD_IN].value--;
		//add current process to dev[STD_IN]'s blocked process list
		...
		asm volatile("int $0x20"); //switch to other process
		int sel = sf->ds;
		char *str = (char *)sf->edx;
		int i = 0;
		int size = sf->ebx;
		...
		while(i < size && bufferHead != bufferTail){
			char c = getChar(keyBuffer[bufferHead]);
